...
export class UpdateCourseUseCase {
    ...
    async execute(id: number, dto: CreateCourseDto) {
        const isCourse = await this.courseRepository.findOne(id);
        if (!isCourse) {
        ...
        const courseExist = await this.courseRepository.findCourseExist(course, id);
